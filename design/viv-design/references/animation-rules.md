# Animation Rules — Citable Values

The precise values, curves, and rules behind animation craft. Cite these in findings rather than approximating. Distilled from Emil Kowalski's design-engineering philosophy ([animations.dev](https://animations.dev/)) and the `review-animations` skill.
...
**Never animate keyboard-initiated actions** — they repeat hundreds of times daily; animation makes them feel slow and disconnected. (Raycast has no open/close animation. That is correct for something used hundreds of times a day.)

Valid purposes for motion: spatial consistency, state indication, explanation, feedback, preventing a jarring change. "It looks cool" on a frequently-seen element is not valid.
...
Find more at [easing.dev](https://easing.dev/) or [easings.co](https://easings.co/). Don't hand-roll from scratch.
...
Feel natural because they simulate physics; no fixed duration — they settle on parameters. Use for:
- drag with momentum
- "alive" elements (Dynamic Island)
...
Mouse interactions: interpolate with `useSpring` rather than tying value directly to mouse position (direct = artificial, no momentum). Only when the motion is decorative.
...
- Orchestration: 30–80ms stagger between sibling items. Lots more reads as broken.
- Staggering within ONE list is legitimate. The tell is the uniform reflex (one identical entrance applied to every section), not motion itself; each reveal should fit what it reveals.
- Suppressing the reflex is never a reason to ship a page with no motion at all. Reveal animations must enhance an already-visible default.

## Reveal animations must enhance an already-visible default

Don't gate content visibility on a class-triggered transition — transitions pause on hidden tabs and headless renderers, so the reveal never fires and the section ships blank. CSS animations also pause when a tab is hidden. The fix: default-state-visible + transition polish, not transition-triggered.
...
Touch devices shouldn't see hover-triggered motion.

## Hover transitions

- 100–160ms with `ease` curves.
- Color, box-shadow, transform changes only. Not position swaps.
- `transition-property: background-color, color, transform, box-shadow` — never `all`.

## Cohesion

Motion matches the component's personality and the rest of the product:
- Playful → bouncier springs allowed; drag-to-dismiss may have momentum.
- Dashboard / tool → crisp, uniform 150–250ms transitions, springs only for gestures.
- Editorial / brand → orchestrated page-load reveals are appropriate; restraint elsewhere.
- Mismatched personality is a finding.
