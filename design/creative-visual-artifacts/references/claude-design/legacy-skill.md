...
- for long dashboards or scroll pages, capture/inspect at least one lower section beyond the hero so chart/card population is verified, not just the first fold
- for JS-rendered data artifacts, verify embedded data parses; when using `<script type="application/json">`, embed raw JSON and only escape closing tags (`</` → `<\/`) rather than HTML-escaping quotes
- for WSL-hosted standalone HTML artifacts, render through Windows Chrome when useful: serve with `python3 -m http.server`, screenshot/PDF via PowerShell, verify file sizes, then inspect the PNG preview with vision. See `references/wsl-windows-chrome-rendering.md`.
