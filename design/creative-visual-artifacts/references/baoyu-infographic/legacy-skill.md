...
- If image generation is unavailable due to missing credentials/provider setup (e.g. `FAL_KEY environment variable not set`) or if exact text/map legibility matters more than painterly output, create a deterministic SVG/HTML/Pillow-rendered PNG infographic; see `references/svg-infographic-fallback.md`. Deterministic rendering is especially appropriate for batch “one infographic per record/video” jobs where readable text, speed, and low cost matter more than painterly illustration.
- If the user asks for a “consultant-level,” “million-dollar,” editable, HTML, email-ready, or recurring/batch infographic, prefer a deterministic self-contained HTML/SVG/Pillow artifact over a pure image-model render. Preserve the same sections/content, use large editorial typography, include a PNG preview and PDF export when useful, and verify the rendered screenshot/sample before emailing or delivering. For WSL → Windows Chrome export mechanics, see `claude-design/references/wsl-windows-chrome-rendering.md`.
- Verify final image/HTML preview legibility with `vision_analyze` before delivery, especially checking for cropped labels, crowded sidebars, unreadable map text, and whether phone/Telegram viewing changes the best layout. For walking-tour maps, prefer a portrait/mobile map-above-checklist version when a landscape sidebar layout is too dense.
