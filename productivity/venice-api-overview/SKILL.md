...
## Hermes skill-library setup notes

If configuring Venice.ai skills in Hermes, see [`references/hermes-venice-skills-setup.md`](references/hermes-venice-skills-setup.md). Key lessons: install upstream skills directly from raw `SKILL.md` URLs when tap search lags, store the same bearer key under both `VENICE_API_KEY` and `VENICE_INFERENCE_KEY` for compatibility with older scripts, verify with `GET /models`, and never hard-code raw Venice keys into skill files.
